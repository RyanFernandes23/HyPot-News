...
async def run_audio_generation_job():
    ...
    try:
        ...
        response = (
            ...
            .select("*")
            .or_(
                "audio_status.eq.pending,and(audio_status.like.failed%,audio_attempts.lt.3)"
            )
            .limit(10)
            .execute()
        )
        ...
        logger.info(
            f"[Audio Job] Found {len(articles)} pending articles. Starting batch process..."
        )
...
async def run_briefing_audio_prep():
    """
    Scheduled job to prepare audio for daily briefing.
    Uses daily_briefing_articles table (from GNews).
    Fetches today's articles per category and processes audio.
    """
            ...
            if response.data:
                for article in response.data:
                    if article.get("audio_status") == "ready":
                        ...
                        continue
                    logger.info(
                        f"[Briefing Prep] Processing audio for: {article.get('headline')}"
                    )
                    try:
                        await audio_processor.process_daily_briefing_article(article)
                        tasks_processed += 1
                    except Exception as article_err:
                        logger.error(
                            f"[Briefing Prep] Failed for article {article.get('id')}: {article_err}"
                        )
            ...
            for entry in os.listdir(temp_dir):
                ...
                if os.path.isdir(path):
                    ...
                    if now - mtime > retention_seconds:
                        logger.info(
                            f"[Cleanup Job] Removing old temp directory: {path}"
                        )
                        shutil.rmtree(path)
